...
function meanVsMedian(numbers) {
    if(n.reduce((a,b) => a + b,0)/numbers.length > n.sort((a,b) => b-a)[1]) return 'mean';
    if(n.reduce((a,b) => a + b,0)/numbers.length < n.sort((a,b) => b-a)[1]) return 'median';
    if(n.reduce((a,b) => a + b,0)/numbers.length === n.sort((a,b) => b-a)[1]) return 'same';
}
...
function swapHeadAndTail(arr) {
    if (arr.length % 2 != 0) {
        ...
        let kozep = arr.splice(index, 1)
        for (let i = 0; i < arr.length/2; i++) {
            arr.push(arr.shift())
        }
        arr.splice(index, 0, ...kozep)
        }
        else {
            for (let i = 0; i < arr.length/2; i++) {
            arr.push(arr.shift())
